...
class GraphQLSchemaService {
   ...
   * @returns {string}
   */
  static getGithubAirdropPost(postId, commentsPage = 1, commentsPerPage = 10) {
    return `
      query {
        ${this._getGithubAirdropPostQuery(postId, commentsPage, commentsPerPage)}
      }
    `
  }
   ...
   * @private
   */
  static _getGithubAirdropPostQuery(postId, commentsPage = 1, commentsPerPage = 10) {
    return `
      one_post_offer(id: ${postId}, comments_query: {page: ${commentsPage}, per_page: ${commentsPerPage}}) {
         ...
         ${PostsQueries.getGithubAirdropPostFields()}

...
const OrgQueries = require('./queries/organizations-queries');
...
class GraphQLSchemaService {
  ...
  static _getManyOrganizationsPageQuery(
    ...
    return `
      query {
        ${OrgQueries.getManyOrganizationsQueryFields(stringFilter, ordering, page, perPage)}
        ${usersQuery}
      }
    `;
  }
    ...
    return `
      query {
        organizations(order_by: "${ordering}", page: ${page}, per_page: ${perPage}) {
          ${OrgQueries.getOrganizationsListFieldsQuery()}
        }
      }
    `
  }
  ...
  static _getCommentsListFields(myselfData) {
    return `
      data {
        ...
        ${OrgQueries.getOrganizationPreviewQuery()}

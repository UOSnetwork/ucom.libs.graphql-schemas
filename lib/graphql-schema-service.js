...
class GraphQLSchemaService {
   ...
   * @returns {string}
   */
  static getManyOrganizationsForTrendingPostsQuery(page, perPage) {
    const filter = {
   ...
   * @returns {string}
   */
  static getManyOrganizationsForHotPostsQuery(page, perPage) {
    const filter = {
   ...
   * @returns {string}
   */
  static getManyOrganizationsForFreshPostsQuery(page, perPage) {
    const filter = {
   ...
   * @returns {string}
   */
  static getManyOrganizationsForTopPostsQuery(page, perPage) {
    const filter = {
  ...
  static getManyTrendingPostsQuery(postTypeId, postPage, postPerPage, commentsPage, commentsPerPage, isMyself = true) {
    const { stringFilter, ordering } = Helpers.getFilterAndOrderingForTrending({
  ...
  static getManyHotPostsQuery(postTypeId, postPage, postPerPage, commentsPage, commentsPerPage, isMyself = true) {
    const { stringFilter, ordering } = Helpers.getFilterAndOrderingForHot({
      post_type_id: postTypeId,
    });
  ...
  static _getManyOrganizationsPageQuery(
    ...
    let usersQuery = '';
    if (togetherWith.users) {
      usersQuery = UsersQueries.getManyUsersQueryFields(togetherWith.users.filter, USERS_ORDERING_FOR_OVERVIEW, 1, 10, isMyself);
    }

    return `
      query {
        ${OrgsQueries.getManyOrganizationsQueryFields(stringFilter, ordering, page, perPage)}
        ${usersQuery}
      }
    `;
  }
   ...
   * @returns {string}
   */
  static _getManyPostsQuery(stringFilter, ordering, relatedFilter, postPage, postPerPage, commentsPage, commentsPerPage, isMyself = true) {
    const commentsQuery = this._getCommentsInputQuery(commentsPage, commentsPerPage);

    const relatedEntityPage     = 1;
    const relatedEntityPerPage  = 10;

    const relatedStringFilter = Helpers.stringify(relatedFilter);

    const relatedEntityOrder = ORDER_BY_CURRENT_RATE_DESC;

    return `
      query {
        ${GraphQLSchemaService._getManyPostsQueryFields(stringFilter, ordering, postPage, postPerPage, commentsQuery, isMyself)}
        ${UsersQueries.getManyUsersQueryFields(relatedFilter, relatedEntityOrder, relatedEntityPage, relatedEntityPerPage, isMyself)}
        ${OrgsQueries.getManyOrganizationsQueryFields(relatedStringFilter, relatedEntityOrder, relatedEntityPage, relatedEntityPerPage)}
   ...
   * @private
   */
  static _getManyPostsQueryFields(stringFilter, ordering, postPage, postPerPage, commentsQuery, isMyself = true) {
    return `
  ...
  static getManyFreshTagsQuery(page, perPage, isMyself = true) {
    ...
    const stringFilter = Helpers.stringify({});
  ...
  static getManyTrendingOrganizationsQuery(page, perPage, isMyself = true) {
    ...
    return this._getManyOrganizationsPageQuery(stringFilter, ordering, page, perPage, isMyself, togetherWith);
  }
  ...
  static _getManyTagsQuery(stringFilter, ordering, usersFilter, page, perPage, isMyself = true) {
    ...
    return `
      query {
        ${TagsQueries.getManyTagsQueryFields(stringFilter, ordering, page, perPage, isMyself = true)}
        ${UsersQueries.getManyUsersQueryFields(usersFilter, USERS_ORDERING_FOR_OVERVIEW, usersPage, usersPerPage, isMyself)}
      }
    `;
  }
}
